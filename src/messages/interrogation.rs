...
impl From<&[u8]> for Interrogation {
    fn from(bytes: &[u8]) -> Self {
        ...
        let type1_1 = get_bits_u8::<70, 6>(bytes);
        let offset1_1 = get_bits_u16::<76, 12>(bytes);
        ...
        let (type1_2, offset1_2) = if payload_bits >= 110 {
            (
                Some(get_bits_u8::<90, 6>(bytes)),
                Some(get_bits_u16::<96, 12>(bytes)),
            )
        } else {
            (None, None)
        };
        ...
        let (mmsi2, type2_1, offset2_1) = if payload_bits >= 160 {
            (
                Some(get_bits_u32::<110, 30>(bytes)),
                Some(get_bits_u8::<140, 6>(bytes)),
                Some(get_bits_u16::<146, 12>(bytes)),
            )
        } else {
            (None, None, None)
        };
