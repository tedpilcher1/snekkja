use crate::messages::{
    ...
    utils::{decode_text, get_bit, get_bits_u8, get_bits_u16, get_bits_u32},
};
...
impl From<&[u8]> for StaticVoyageData {
    fn from(bytes: &[u8]) -> Self {
        ...
        let callsign = decode_text(bytes, 70, 7);
        let shipname = decode_text(bytes, 112, 20);
        ...
        let destination = decode_text(bytes, 302, 20);
