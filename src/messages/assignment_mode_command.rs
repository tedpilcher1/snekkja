...
impl From<&[u8]> for AssignmentModeCommand {
    fn from(bytes: &[u8]) -> Self {
        ...
        let offset1 = get_bits::<u16, 70, 12>(bytes);
        let increment1 = get_bits::<u16, 82, 10>(bytes);

        let payload_bits = bytes.len().saturating_sub(7) * 8;

        let (mmsi2, offset2, increment2) = if payload_bits >= 144 {
            (
                Some(get_bits::<u32, 92, 30>(bytes)),
                Some(get_bits::<u16, 122, 12>(bytes)),
                Some(get_bits::<u16, 134, 10>(bytes)),
            )
        } else {
            (None, None, None)
        };
